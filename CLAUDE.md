...
`modules/common.nix` is the always-on aggregator (imports `shell.nix`, `git.nix`, `dev.nix`; sets `stateVersion` and `programs.home-manager.enable` once). Everything else is opt-in via a host's `imports` list — Linux hosts take `gui.nix` + `linux.nix`, the Mac takes `gui.nix` + `darwin.nix`.
...
- **Local overrides are intentional.** `modules/shell.nix` sources `~/.zshrc.local`, `~/.bashrc.local`, `~/.profile.local`, `~/.zprofile.local` if they exist. These are never managed by Home Manager — they're where machine-specific or private config goes. Don't move their contents into the repo.
