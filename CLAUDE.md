...
- **Karabiner config is seeded once.** `darwin.nix` copies `configs/karabiner.json` into `~/.config/karabiner/` only if the file doesn't already exist — Karabiner rewrites that file at runtime, so it's deliberately not managed declaratively after the first run.
