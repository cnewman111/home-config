...

{
  ...
  #
  # Uses the home-manager binary this config installs rather than
  # `nix run home-manager/master`: the short name is a flake-registry lookup,
  # and a registry that replaces the public one (as on the work machines) has
  # no home-manager entry, so the short form fails there.
